package bg.avi.numrec.web.admin.service;

import java.lang.reflect.Type;
import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import org.springframework.stereotype.Service;

import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;

import bg.avi.numrec.web.admin.config.Config;
import bg.avi.numrec.web.admin.dto.PlatePatternDTO;
import bg.avi.numrec.web.admin.util.HTTPRequestUtil;

@Service
public class PlatePatternService {
	
	public List<PlatePatternDTO> getAllPatterns() {
		List<PlatePatternDTO> list = Collections.emptyList();	
		try {
			String jsonResponse = HTTPRequestUtil.doGet( Config.APPLICATION_BASE_PATH + "/platePatterns/findAll", "da", "da");
			if (jsonResponse != null) {
				Type listType = new TypeToken<ArrayList<PlatePatternDTO>>() {}.getType();
				list = new Gson().fromJson(jsonResponse, listType);
			}
		} catch (Exception e) {
			// Tujno
		}
		
		return list;
	}

	public List<PlatePatternDTO> getPatternsByExample(PlatePatternDTO searchObject) {
		List<PlatePatternDTO> list = Collections.emptyList();
		if (searchObject.isEmpty()) {
			list = getAllPatterns();
		} else {
			searchObject.removeEmpty();
			try {
				String jsonResponse = HTTPRequestUtil
						.doGetWithBody(Config.APPLICATION_BASE_PATH + "/platePatterns/find", new Gson().toJson(searchObject), "da", "da");
				if (jsonResponse != null) {
					Type listType = new TypeToken<ArrayList<PlatePatternDTO>>() {}.getType();
					list = new Gson().fromJson(jsonResponse, listType);
				}
			} catch (Exception e) {
				// Tujno
			}
		}
		
		return list;
	}

	public void saveOrUpdate(PlatePatternDTO addEditObject) {
		try {
			HTTPRequestUtil.doPost(Config.APPLICATION_BASE_PATH + "/platePatterns/update", new Gson().toJson(addEditObject), "da", "da");
		} catch (Exception e) {
			// Tujno
		}
	}

}
